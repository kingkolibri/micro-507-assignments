...
num_steps = size(sln.Y,2); % total number of steps the robot has taken (find this from sln)
r0 = [0; 0];
tic();
for j = 1:num_steps
    Y = sln.Y{j};
...
% of 1 mean?
t_real = sln.T{1}(end) - sln.T{1}(1); % This is only an estimation 
real_time_factor = t_real / t_anim;
